...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Selector used across metas/calendar logic to identify the "sueño" card
  // include the explicit data-id used in `pages/metas.html`
  const SLEEP_META_SELECTOR = '.card[data-id="meta-bienestar"], .card[data-id*="sueno"], .card[data-id*="sueño"], .card[data-id*="sleep"], .card.meta-sleep';
  ...
  // Only manage a single meta: "sueño".
  // Using the global SLEEP_META_SELECTOR defined above (includes meta-bienestar).

    const calendar = loadCalendar();
    const todayKey = isoDate(new Date());

    const card = document.querySelector(SLEEP_META_SELECTOR);
    if (!card) return; // nothing to do if sleep card not present
    ...
    if (btn) {
      btn.addEventListener('click', () => {
        // reload calendar state and toggle only today's date for this single meta
  ...
  function renderCalendar() {
    ...
    const sleepCard = document.querySelector(SLEEP_META_SELECTOR);
    const sleepId = sleepCard ? sleepCard.getAttribute('data-id') : null;
    ...
    for (let d = 1; d <= last.getDate(); d++) {
      ...
      // Only consider stored meta ids for this date. If sleepId is set, filter
      // all meta ids to only include the sleep meta so the calendar visually
      // shows a single meta.
      let metaIds = Array.isArray(cal[key]) ? cal[key] : [];
      if (sleepId) metaIds = metaIds.filter(id => id === sleepId);
